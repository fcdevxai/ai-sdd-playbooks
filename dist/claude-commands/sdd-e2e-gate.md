# /sdd-e2e-gate — SDD E2E Gate — Backend Integration Validation via Browser
...
After `/sdd-code-review`. Can run in parallel with, or after, `/sdd-ux-gate`, always before `/sdd-commit`. Only applies to projects with a web UI — self-skips automatically in backend-only projects (REST APIs with no UI).
