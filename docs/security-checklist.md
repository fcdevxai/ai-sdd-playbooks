...
| ~~`postinstall` (`scripts/postinstall.cjs`)~~ — eliminado | El paquete ya no declara ningún lifecycle script de npm: nada que npm pueda ejecutar automáticamente en la máquina o CI de un consumer al instalar o actualizar. ADR-034 (message-only) queda superseded por `adr-remove-postinstall-lifecycle-script.md`, que documenta la reproducción empírica del fallo que motivó la eliminación. La señal post-install ahora vive fuera de npm: el README documenta el comando de instalación real, y el CLI (`playbook`/`sdd`) imprime un aviso autoextinguible cuando los skills globales no están instalados. | maintainers |
...
| El checklist de evidencia real del adapter `worker` no tiene enforcement mecánico, y este repo no puede dogfoodearlo | ADR-041: `worker` pasa de experimental a supported sin dependencia declarada (mismo modelo que `http`). Nada verifica que el agente haya disparado de verdad la cola real en vez de invocar el handler a mano — el checklist es prosa, igual que en `browser`/`http`. Además `playbook-ai` declara `capabilities.worker: false`, así que este repo no ejerce el adapter contra un worker propio; la primera corrida real ocurre en un proyecto consumidor. Mitigado por exigir el camino de disparo/consumo real explícitamente en el checklist y por citar el `AC-N` validado en cada hallazgo — mismo estándar de corroboración que `http`/`browser`. | maintainers (ADR-041) | 2026-09-02 |
...
| ~~Con `--ignore-scripts`, el `postinstall` message-only no corre — sin señal de post-update~~ — resuelto | ADR-034 (el trade-off aceptado de mantener un `postinstall`) queda superseded por `adr-remove-postinstall-lifecycle-script.md`: sin lifecycle script, `--ignore-scripts` ya no cambia nada — no hay ningún script que la flag pudiera ignorar. | maintainers (ADR-034, superseded) | 2026-07-27 |
