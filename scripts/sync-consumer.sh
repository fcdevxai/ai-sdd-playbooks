...
#   CREATE_DOC_COMMANDS    default: (interactive)   set to "yes" or "no" to skip prompt for add-on documentation commands
...
# ──────────────────────────────────────────────────────────────────────────────
#  Sync mode — copy SDD core commands
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
#  Add-on commands (documentation for Confluence)
# ──────────────────────────────────────────────────────────────────────────────
ADDON_SLUGS=("document-code" "write-in-confluence")
MISSING_ADDONS=()

for slug in "${ADDON_SLUGS[@]}"; do
  src="${SUBMODULE_DIR}/playbooks/${slug}/canonical.md"
  dest="${COMMANDS_DEST}/${slug}.md"
...
if [[ ${#MISSING_ADDONS[@]} -gt 0 ]]; then
  echo "⚠️  Comandos add-on de documentación disponibles pero no instalados:"
  for slug in "${MISSING_ADDONS[@]}"; do
    echo "   - ${COMMANDS_DEST}/${slug}.md"
  done
  echo ""
  echo "   Estos comandos añaden automatización de documentación técnica para Confluence:"
  echo "   - /document-code        → documenta entidades, servicios y componentes en Confluence"
  echo "   - /write-in-confluence  → redacta guías operacionales en Confluence"
  echo ""

  if [[ -n "${CREATE_DOC_COMMANDS:-}" ]]; then
    if [[ "${CREATE_DOC_COMMANDS,,}" == "yes" || "${CREATE_DOC_COMMANDS,,}" == "y" ]]; then
      COPY_ADDONS=true
    else
      COPY_ADDONS=false
    fi
  else
    read -rp "¿Deseas instalar los comandos de documentación? (s/n): " response
...
if [[ ${#MISSING_DOCS[@]} -gt 0 ]]; then
  ...
  echo ""
  echo "   Estos archivos son requeridos para que los comandos SDD funcionen correctamente."
  echo ""
  ...
  if "$CREATE_THEM"; then
    ...
    echo ""
    echo "  ⚠️  IMPORTANTE: Los comandos SDD dependen de estos archivos para tener contexto del proyecto."
...
if [[ ${#MISSING_CLAUDE[@]} -gt 0 ]]; then
  ...
  if "$CREATE_CLAUDE"; then
    ...
    echo "     El bloque SDD entre marcadores será administrado automáticamente por sync-consumer.sh"
    echo ""
  else
    echo ""
