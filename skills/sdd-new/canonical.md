---
name: sdd-new
description: "Create the SDD change folder and initial artifacts (OWNER.md, a schema-valid proposal.md draft, tasks.md placeholder, ADR drafts for flagged decisions) under openspec/changes/ from a decision-closed requirement produced by sdd-enrich-us. Activate when the user says 'sdd-new', 'create change folder', or wants to scaffold openspec/changes/ after enrich. Triggers: crea el change, materializa la proposal."
description_es: "Crear la carpeta de change y sus artefactos iniciales (OWNER.md, proposal.md borrador válido contra schema, tasks.md placeholder, drafts de ADR para decisiones marcadas) a partir de un requisito con decisiones cerradas producido por sdd-enrich-us."
title_en: "SDD New — Create Feature Artifacts"
title_es: "SDD New — Crear Artefactos de Feature"
when: "After `sdd-enrich-us` produces a decision-closed requirement. Before `sdd-design`/`sdd-plan`."
output_file: "OWNER.md, proposal.md, tasks.md (placeholder), adr-*.md (drafts, one per [ADR candidate] decision)"
...
the affected domain spec if one exists, and `docs/security-checklist.md` (known
sensitive surfaces and project security rules).
