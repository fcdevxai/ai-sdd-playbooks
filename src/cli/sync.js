/**
 * `playbook sync` (design §1.2).
 *
 * Reconciles `playbook.lock`'s `resolved` with the installed global methodology
 * version (C-08 / R-01).
 */
...
export async function syncCommand(parsed, io) {
  ...
  if (!lock) { io.out('No playbook.lock in this project; nothing to reconcile.'); return EXIT.OK; }
  if (!installed) {
    io.err('No global methodology installed (run `playbook install`).');
  ...
  io.out(`Reconciled: playbook.lock resolved → ${installed} (compatible: ${lock.methodology.compatible}).`);
  return EXIT.OK;
}
