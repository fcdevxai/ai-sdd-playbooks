/**
 * playbook.config.yaml IO and merge (design §5.1).
 *
 * Precedence: package defaults → playbook.config.yaml → env overrides.
...
export const ENV_OVERRIDES = [
  ['PLAYBOOK_METHODOLOGY_COMPATIBLE', ['methodology', 'compatible']],
  ['PLAYBOOK_GITHUB_BASE_BRANCH', ['github', 'base_branch']],
  ['PLAYBOOK_PROJECT_LANGUAGE', ['project', 'language']],
];
...
export function configPathFor(cwd, configPath) {
  return configPath || path.join(cwd, 'playbook.config.yaml');
}
...
export function validateConfig(config) {
  return validateNamed('playbook.config', config);
}
