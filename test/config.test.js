...
test('mergeConfig: env overrides file (precedence)', () => {
  const merged = mergeConfig(
    ...
    { SDD_GITHUB_BASE_BRANCH: 'release' },
  );
...
test('loadConfig reads and merges a real sdd.config.yaml', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-cfg-'));
  const file = path.join(dir, 'sdd.config.yaml');
