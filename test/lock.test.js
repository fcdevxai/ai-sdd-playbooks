...
test('buildLock records compatible + resolved, not a bare version (C-08)', () => {
  const lock = buildLock({ compatible: '>=0.1.0 <1.0.0', resolved: '0.1.3', installedAt: '2026-07-22' });
  assert.equal(lock.version, 2);
  assert.equal(lock.methodology.compatible, '>=0.1.0 <1.0.0');
  assert.equal(lock.methodology.resolved, '0.1.3');
...
test('lock round-trips through write/read', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'playbook-lock-'));
  const file = path.join(dir, 'playbook.lock');
  const lock = buildLock({
    compatible: '>=0.1.0 <1.0.0',
    resolved: '0.1.3',
    skills: { 'sdd-plan': { version: '0.1.3' } },
