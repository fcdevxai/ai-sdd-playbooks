...
const SCRIPT = path.join(REPO_ROOT, 'scripts', 'postinstall.cjs');

function tmp() { return fs.mkdtempSync(path.join(os.tmpdir(), 'playbook-postinstall-')); }

test('package.json declares exactly one lifecycle script, postinstall, pointing at scripts/postinstall.cjs, with prepare/preinstall absent (AC-10)', () => {
...
test('running scripts/postinstall.cjs exits 0, prints at most 3 lines, and names `playbook install` (AC-10)', () => {
...
test('copied to a directory where its package.json does not resolve, it exits 0 and prints nothing (EC-4)', () => {
  const dir = tmp();
  const copy = path.join(dir, 'postinstall.cjs');
...
test('the script source imports nothing from src/, performs no filesystem writes or network calls, and never re-throws (SEC-001)', () => {
  ...
  assert.doesNotMatch(src, /require\(['"].*\/src\//, 'no require from src/');
  assert.doesNotMatch(src, /writeFile|mkdir|(?<!copy)[Rr]m\(|copyFile|appendFile/, 'no filesystem-write calls');
  assert.doesNotMatch(src, /\bfetch\(|require\(['"]https?['"]\)|require\(['"]net['"]\)/, 'no network calls');
