...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'playbook-validate-'));
...
## Acceptance criteria
AC-01: something testable.

## Error cases
EC-01: something fails gracefully.

## Security considerations
Not applicable: no sensitive surface touched.
...
test('validate: a proposal body missing a required section is a violation', async () => {
  const dir = makeRepo();
  writeChange(dir, 'proposal.md', VALID_PROPOSAL.replace('## Security considerations\nNot applicable: no sensitive surface touched.\n\n', ''));
...
test('validate: a present context-packet.md missing a required section is a violation', async () => {
  const dir = makeRepo();
  writeChange(dir, 'proposal.md', VALID_PROPOSAL);
  writeChange(dir, 'context-packet.md', '# Context Packet — Demo\n\n## Ticket\n\ndemo\n');
  const { io, err } = capture();
  const code = await run(['validate', '--cwd', dir], io);
  assert.equal(code, EXIT.VIOLATION);
  assert.match(err.join('\n'), /context-packet\.md: missing section/);
});
...
// Per-change relevance cross-check: only active when the proposal declares
// `runtime_relevant_capabilities`; project has worker:true throughout.
